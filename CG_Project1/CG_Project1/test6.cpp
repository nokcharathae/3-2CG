...
#include <vector>
#include <map> // 여러개의 오브젝트를 추가 하였음. 랜더링을 다한 다음에 새로 만든것을 다시 랜더링하는 과정을 거쳤다. 
#include <string>
...
ID3D11Texture2D* g_texEnvMap = nullptr; //setting
...
public:
    ...
    void UpdateConstanceBuffer() {// 각각의 새로운 오브젝트에 대해서 matrix들을 넘겨준다. 
        struct ModelConst {
            Matrix mModel;
...
std::map<std::string, MyObject> g_sceneObjs; //map == 딕셔너리라고 생각하면 된다. string 이 key 뒤에게 값이라고 생각하면 된다.
// key가 식별자로 사용된다.
...
#pragma region Create a cube
    {
        ...
        // Create index buffer 인덱스를 가지고 튜브를 만들었음
        WORD indices[] =
        {
           3,1,0,
           2,1,3,
        ...
        //쉐이더를 가리키는 View를 만들어야 함
        D3D11_SHADER_RESOURCE_VIEW_DESC dcSrv = {};
        dcSrv.Format = DXGI_FORMAT::DXGI_FORMAT_R32G32B32_FLOAT; // 형식
...
#pragma region Create a sphere 
    { // 구를 만드는 코드.
        ...
        const int stackCount = 100, sectorCount = 100; // 얼마나 정밀하게 할것인가.
        GenerateSphere(0.5f, sectorCount, stackCount, position, normal, texcoord); // 원점은 중심, 지름은 0.5
...
#pragma endregion Create a sphere
    ...
    bdCB.ByteWidth = sizeof(CB_TransformScene); // 씬에 공통으로 사용되는 메트릭스를 빼고 저장. projection, camera..(맞나?? 확인해 보자.)
...
#pragma region Env Mapping 
    //환경맵 읽어오기
    int w, h, n;
    unsigned char* img = stbi_load("envimap1.jpg", &w, &h, &n, 0);
    ...
    dcTex2D.ArraySize = 1;
    dcTex2D.MipLevels = 1;   //mipmap 읽어오기
    dcTex2D.SampleDesc.Count = 1;
    dcTex2D.SampleDesc.Quality = 0;
    dcTex2D.Usage = D3D11_USAGE_IMMUTABLE;
    //defult는 gpu 에서 r,w이 되는것 / immutable r만 가능 변하는게 없을때 좋다 / 다이나믹 gpu는 r cpu는 w constant buffer 용으로 많이 사용된다 근데 또 간혹 일어나는 이벤트는 defult가 좋다
    ...
    InitTex2DData.SysMemPitch = w * 4; //strid? 정보
...
#pragma endregion
    ...
    D3D11_SAMPLER_DESC dcSamlpe = {};
    dcSamlpe.Filter = D3D11_FILTER::D3D11_FILTER_MIN_MAG_MIP_LINEAR;
    dcSamlpe.AddressU = D3D11_TEXTURE_ADDRESS_MODE::D3D11_TEXTURE_ADDRESS_WRAP;
    dcSamlpe.AddressV = D3D11_TEXTURE_ADDRESS_MODE::D3D11_TEXTURE_ADDRESS_WRAP;
    dcSamlpe.AddressW = D3D11_TEXTURE_ADDRESS_MODE::D3D11_TEXTURE_ADDRESS_WRAP;
    *(Vector4*)&dcSamlpe.BorderColor = Vector4();
    dcSamlpe.MipLODBias = 0;
    dcSamlpe.MaxAnisotropy = 16;
    dcSamlpe.MinLOD = 0;
    dcSamlpe.MaxLOD = D3D11_FLOAT32_MAX;
    dcSamlpe.ComparisonFunc = D3D11_COMPARISON_NEVER;
    hr = g_pd3dDevice->CreateSamplerState(&dcSamlpe, &g_samplerTex2D);
    if (FAILED(hr)) {
        return hr;
    }
    ...
    // 전역변수로 맵 설정했음. // 하나더 만들고 싶으면 똑같이 만든 다음에 이름만 바꾸어주면된다. 
    g_sceneObjs["STL"] = MyObject(g_pVertexBuffer_stl, g_pIndexBuffer_stl, g_pSRV_stl, g_pIALayoutP, g_pVertexShaderP, g_pPixelShader4,
        ...
        Matrix::CreateScale(1.f / 12.f) * Matrix::CreateTranslation(-10.f, 0.f, 0.f), Color(0.5f, 0.5f, 0.5f), Color(0.7f, 0.f, 0.7f), Color(0.2f, 0, 0.2f), 10.f);
    ...
    //   g_pRSState, g_pDSState, sizeof(CubeVertex), sizeof(WORD), indices_cube,
    //   Matrix::CreateScale(20.f)*Matrix::CreateTranslation(10,0,0), Color(0.1f, 0.1f, 0.1f), Color(0.7f, 0.7f, 0), Color(0.2f, 0, 0.2f), 10.f);
    ...
    //배경용 추가
    Matrix matR = Matrix::CreateLookAt(Vector3(0, 0, 0), Vector3(0, -1, 0), Vector3(0, 0, 1));//보는방향 재설정
...
#pragma region Common Scene (World)
    ...
    //g_pImmediateContext->VSSetConstantBuffers(2, 1, &g_pCB_Lights); 고라우드 하고 싶으면 넣으면 되지만 필요없다. 지금음
...
#pragma endregion Common Scene (World)
    ...
    for (auto& pair : g_sceneObjs) // for each문으로 되어 있음. 명시적으로 적으면 안되는 버그가 있는거같음. auto는 알아서 타입을 자동으로 맞추어준다. 
    {
        //if (pair.first == "CUBE") continue;
        MyObject& obj = pair.second;// second는 벨류.
#pragma region For Each Object
      // Set vertex buffer
        ...
        // Set index buffer
        switch (obj.ib_stride)// 사이즈에 따라서 명시적으로 지정해주는 것이라고 보면된다. 
        {
        case 2:
        ...
        obj.UpdateConstanceBuffer(); // 바뀐값을 적용하게 하기 위해서 호출을 해주었음. 
        ...
        //g_pImmediateContext->DrawIndexed(6,6,0) 으로 두번째 그려지는 면 확인가능, 컬링을 끄고하는게 편함.  아래 이프엘스 삭제하고 해야함
        // 일일이 그려보면서 해당면의 앞면이 어디인지를 알아볼 수 있다. +x, -z 등등
...
void CleanupDevice()
{
    for (auto& obj : g_sceneObjs) obj.second.Delete(); // 맵에 있는 모든 오브젝트에 대해서 삭제를 진행하고 아래에서 릴리즈. 
    g_sceneObjs.clear();
    ...
    if (g_texEnvMap) g_texEnvMap->Release(); //setting 
