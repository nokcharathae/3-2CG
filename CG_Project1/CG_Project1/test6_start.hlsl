...
float4 PS1(PS_INPUT1 input, uint primID : SV_PrimitiveID) : SV_Target
{
    ...
    //if (primID < 4 && primID >=2) // 프래그먼트가 속하는 primID를 확인할 수 있음. 
...
float4 PS4(PS_INPUT1 input, uint primID : SV_PrimitiveID) : SV_Target
{
    ...
    float3 vr = mul(vR, g_mView2EnvOS);
    vr = normalize(vr);

    float phi = asin(vr.z);
    float cosphi = cos(phi);
    float theta = acos(max(min(vr.x / cos(phi),0.999f),-0.999f));
    float u = vr.y >= 0 ? theta / (2 * X_PI) : (2 * X_PI - theta) / (2 * X_PI);
